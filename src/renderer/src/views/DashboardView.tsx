    ...
    <div className="dashboard-app">
      <div className="dashboard-frame">
        ...
        <main className="dashboard-main">
          ...
          {/*
            The writing surface, on its own rail entry.

            The dashboard design has no route to it — its Home is "paste a
            claim" — but this is where the Essay Grade report, the inline
            underlines, the hover popovers and Find Evidence all live, so the
            shell was adopted with one nav item added rather than leaving the
            largest part of the product unreachable.

            `sourceTypes`/`onAnalysisCreated` were props the dashboard branch
            added to AnalyzeView back in August; that version of the file no
            longer exists. Today's AnalyzeView owns its own source picker, so
            it takes the navigation callback and nothing else.
          */}
          {page === 'documents' || page === 'new-session' ? (
            <AnalyzeView onNavigate={() => navigate('home')} />
          ) : null}
