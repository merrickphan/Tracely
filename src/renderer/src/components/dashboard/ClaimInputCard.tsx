...
export default function ClaimInputCard({
  ...
  async function pasteFromClipboard(): Promise<void> {
    ...
    setPasteError(null)
    try {
      // The renderer's own clipboard API, NOT a bridge call.
      //
      // This originally read through `tracelyApi.readClipboard()`. That channel
      // does not exist any more and must not come back: preload deliberately
      // dropped `clipboard.read` because it had no renderer callers (the global
      // hotkey reads the clipboard main-side) and was therefore an unnecessary
      // read-the-user's-clipboard capability sitting on the bridge. Re-adding a
      // privileged channel to power a convenience button would undo that for no
      // gain — this reads only when the button is pressed, and Ctrl+V into the
      // textarea does the same job with no clipboard access at all.
      const text = await navigator.clipboard.readText()
      onTextChange(text.slice(0, MAX_CLAIM_CHARACTERS))
